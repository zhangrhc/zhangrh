...
class LoginCase(unittest.TestCase):
    ...
    def test_validate_button(self):
        ...
        sleep(1)
        #选择平行的两组元素中第二个，所有的li标签id='pmng_admin'中第二个。机构版档案对照、项目验证
        self.dr.find_element_by_xpath("//li[2][@id='pmng_admin']/a/span").click()
        sleep(1)
        link11=self.dr.find_element_by_id('0001ZZ1000000001RYH5').text
        self.assertTrue('机构档案对照' in link11)
        link12=self.dr.find_element_by_id('0001ZZ10000000022502').text
        self.assertTrue('项目' in link12)
        #工作日志权限控制验证
        ...
        sleep(1)
        link4=self.dr.find_element_by_id('0001ZZ1000000001SWFA').text
        self.assertTrue('自动验证规则设' in link4)
